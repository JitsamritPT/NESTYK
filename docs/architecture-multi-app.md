...
## 1.1 App-First Ecosystem & Deep Linking Strategy

ระบบใช้โมเดล **App-First Strategy (Marketplace Web + All-in-One Mobile Super App)** เพื่อประสิทธิภาพการพัฒนา การดูแลรักษา และ UX ที่ดีที่สุด:

| Application | Stack | บทบาทและขอบเขตหน้าที่ |
|---|:---:|---|
| **`apps/web`** (Port 3000) | Next.js 15 (SSR) | **Marketplace Portal:** ค้นหาห้อง, SEO Google, ดูรายละเอียดห้อง และส่งต่อผู้ใช้เข้าแอปด้วย Smart Deep Link (`nestyk://*`) |
| **`apps/consumer-app`** (Port 8081) | Expo React Native 54 | **All-in-One Mobile Super App:** รวมทุกบทบาท (Guest, Tenant, Owner, Agent, Admin, Services) สลับโหมดได้ในแอปเดียวอย่างไร้รอยต่อ |
| **`apps/api`** (Port 4000) | NestJS + Supabase | **Unified Backend API:** ดูแล Auth SSO, PostgreSQL, Storage และ Realtime สำหรับทุกแอปพลิเคชัน |

### Deep Linking Scheme Table (`nestyk://*`)
* `nestyk://guest` ➔ โหมดค้นหาห้องพักบน Native GPS Map
* `nestyk://tenant` ➔ โหมดผู้เช่า ตรวจสอบสัญญาดิจิทัล และบิลค่าเช่า
* `nestyk://owner` ➔ โหมดเจ้าของห้อง ลงประกาศห้องพักด้วย AI
* `nestyk://agent` ➔ โหมดนายหน้า ดูแลสต็อก Co-Broke และค่าคอมมิชชั่น
* `nestyk://admin` ➔ ศูนย์ Dispatcher และรับ Ticket งานบริการ
* `nestyk://services` ➔ ศูนย์บริการส่วนกลาง On-Demand (ช่าง, แม่บ้าน, ตรวจห้อง)
* `nestyk://listing/:id` ➔ เปิดดูรายละเอียดห้องพัก
* `nestyk://viewing/:id` ➔ ส่งคำขอนัดดูห้องล่วงหน้า
