...
async function main() {
  ...
  try {
    ...
    await db.query(fs.readFileSync(path.resolve(__dirname, '../migrations/20260914-contract-signatures.sql'), 'utf8'));
    const rows = await db.query(`
      SELECT column_name FROM information_schema.columns
      WHERE table_schema = $1 AND table_name = 'lease_contracts'
        AND column_name IN ('owner_signature_url', 'tenant_signature_url', 'agent_signature_url')
...
main().catch(error => { console.error('Contract signatures migration failed:', error.code || error.name); process.exitCode = 1; });
