...
async function main() {
  ...
  try {
    ...
    await db.query(fs.readFileSync(path.resolve(__dirname, '../migrations/20260914-reservation-documents.sql'), 'utf8'));
    const rows = await db.query(`
      SELECT column_name
      FROM information_schema.columns
      WHERE table_schema = $1 AND table_name = 'lease_contracts'
        AND column_name IN ('invoice_url', 'receipt_url', 'document_url')
...
main().catch(error => { console.error('Reservation documents migration failed:', error.code || error.name); process.exitCode = 1; });
