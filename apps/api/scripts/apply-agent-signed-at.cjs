...
async function main() {
  ...
  try {
    ...
    await db.query(fs.readFileSync(path.resolve(__dirname, '../migrations/20260914-agent-signed-at.sql'), 'utf8'));
    const rows = await db.query(`
      SELECT column_name, data_type, is_nullable
      FROM information_schema.columns
      WHERE table_schema = $1 AND table_name = 'lease_contracts' AND column_name = 'agent_signed_at'
    `, [schema]);
...
main().catch(error => { console.error('Agent signed-at migration failed:', error.code || error.name); process.exitCode = 1; });
