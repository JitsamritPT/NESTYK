# NESTIQ listing promo — global system prompt
# Output language is injected per request (locale). Do not hardcode a language here.
# Runtime source of truth: apps/api/src/agent/rooms/prompts/listing-promo.system-prompt.ts
# Docs copy kept for review — keep in sync with the API file.
