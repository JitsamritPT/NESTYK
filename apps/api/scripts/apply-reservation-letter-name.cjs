...
async function main() {
  ...
  try {
    ...
    await db.query(fs.readFileSync(path.resolve(__dirname, '../migrations/20260914-reservation-letter-name.sql'), 'utf8'));
    const rows = await db.query("SELECT code, name_th FROM master_agreement_types WHERE code = 'reservation'");
...
main().catch(error => { console.error('Reservation letter name migration failed:', error.code || error.name); process.exitCode = 1; });
